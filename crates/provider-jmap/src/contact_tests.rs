use engine_core::{
    contact::{
        ...
        ContactPatch, ContactProperty, ContactResource, FieldPatch, PropertyId,
    },
...
async fn direct_fetch_patch_delete_and_photo_paths_are_exact() {
    ...
    let photo = provider
        ...
        .await
        .unwrap();
    let photo = photo.expect("the blob downloads");
    assert_eq!(photo.as_bytes(), b"photo-bytes");
    assert_eq!(photo.fingerprint, "blob-1");

    // A card naming a blob the server no longer holds is a photo that is gone, not a
    // download that failed: the caller must be able to remember it and stop asking.
    let executor = FakeExecutor::new(vec![]);
    assert!(
        super::JmapProvider::with_executor(Box::new(executor))
            .fetch_contact_photo(
                &account(),
                &card,
                &ContactResource {

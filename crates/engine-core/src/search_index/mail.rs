...
use super::{
    FtsField, FtsRow, MembershipKind, MembershipRow, mail_refs::MailRefRow, normalize_addr,
    project_refs,
};
use crate::{
...
pub struct MailProjection {
    ...
    /// The message-id graph rows threading is maintained from — empty for a provider-threaded
    /// message, which is not in the graph ([`project_refs`]).
    pub refs: Vec<MailRefRow>,
}
...
pub fn project_message(message: &Message) -> MailProjection {
    ...
    MailProjection {
        ...
        addresses,
        memberships,
        refs: project_refs(&key, &message.envelope, message.thread.as_ref()),
    }
}

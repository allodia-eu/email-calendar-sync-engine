//! Windows time-zone name → IANA zone id, at the Graph adapter boundary.
//!
//! Microsoft Graph reports event zones as **Windows** names (`"W. Europe Standard
//! Time"`, `"Pacific Standard Time"`, `"UTC"`), but the engine time model is
//! **IANA-only** (`calendar-semantics.md`). This maps the Windows name to the IANA
//! zone the engine's bundled tzdb resolves — so a Graph event's wall clock expands
//! and localizes through the same tzdata as every other provider.
//!
...
//! `common/supplemental/windowsZones.xml`. It is the same mapping the .NET/ICU stacks
//! use. A legacy custom zone (`tzone://Microsoft/Custom`) or any name absent from the
//! table returns `None`, so the caller preserves it as a
//! [`TimeZoneId::custom`](engine_core::time::TimeZoneId::custom) rather than guessing
//! (the engine's expander then rejects the custom zone until support lands —
//! `calendar-semantics.md`, "custom/embedded-VTIMEZONE zones").

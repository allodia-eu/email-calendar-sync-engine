...
///
/// The **moved** occurrence is not exercised here, because nothing can write one yet — Google
/// still refuses `PatchTarget::Instance`. Its reading is pinned offline against a *captured*
/// series (`events_series_with_overrides.json`, a real move and a real cancellation), and
/// gets its live assertion when the write lands.
#[tokio::test]
async fn live_calendar_reads_a_removed_occurrence_off_the_series() {
    use core::num::NonZeroU32;

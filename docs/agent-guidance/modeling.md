...
- A mail collection carries the **caller's access rights** (`Mailbox::access: MailboxAccess`), as a calendar does. They are a first-class field because both mail protocols that share mailboxes standardise them (JMAP `MailboxRights`, RFC 8621 §2; IMAP ACL, RFC 4314) and because there is **no usable answer above the collection**: live against Stalwart, a JMAP account shared read-only reports `accounts.<id>.isReadOnly: false` while the one mailbox it exposes grants `lr` alone, so "may I write here?" asked of the account gets the wrong answer.
  - The rights are kept separate rather than collapsed to read/write, because servers hand out arbitrary subsets: a shared mailbox commonly lets each reader keep their own `$seen` without changing other keywords, and IMAP separates adding (`i`) from removing (`t`+`e`) from creating a child (`k`) from deleting the mailbox (`x`).
  - **JMAP's `maySubmit` (and IMAP's `p`) is deliberately not carried.** It means mail may be *posted into* the mailbox by address (RFC 8621 §2: "Messages may be submitted directly to this Mailbox"), no client action depends on it, and its name invites reading it as permission to send *as* the mailbox's owner — which no mailbox right expresses. `may_share` is carried, from IMAP's `a` and JMAP's `mayShare` (which RFC 8621 does not define but sharing servers report), symmetric with `CalendarAccess`.
  - Sourced per adapter: JMAP reads `myRights` (a missing object is `owner()`, a missing right inside one is withheld); IMAP maps the `MYRIGHTS` letters, read off the folder `LIST` where the server has `LIST-MYRIGHTS` (RFC 8440) and otherwise from one pipelined `MYRIGHTS` per selectable folder (`provider-imap`'s `acl` module tabulates the mapping — reading needs `l`+`r`, removing needs `t`+`e`, and RFC 2086's `c`/`d` are ignored because an RFC 4314 server adds them for *any* member right); Graph grants Full Access all-or-nothing per mailbox and Gmail labels carry no rights, so both report `owner()` — correct there, not optimistic.
  - Unknown reads as `owner()`, never as "no rights" — a server with no ACLs has nothing to restrict with, and hiding mail the caller can see is the worse error. The field is `#[serde(default)]` for the same reason, and `NORMALIZER_VERSION` 6 re-snapshots stored mailboxes so none keeps the default in place of the server's answer.

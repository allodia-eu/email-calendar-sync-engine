...
- **Storing a draft is two neutral verbs, and the key is the adapter's answer, not the caller's assumption.** `put_draft(account, &Draft, replacing: Option<&ProviderKey>) -> ProviderKey` and `delete_draft(account, &ProviderKey)`, gated by the `mail_drafts` capability. A host hands over the same `Draft` it would submit, plus the draft this one supersedes, and stores **whatever key comes back** — because on three of the four adapters the key *moves* and on one it does not:
  ...
  | Graph | `PATCH /me/messages/{id}` | **stays** | a draft's `subject`/`body`/recipients are writable, so a text edit is one request |
  | Graph, attachment changed | `POST /me/messages`, then purge | **moves** | `PATCH` cannot touch the attachment collection, and there is no MIME update |
  | Gmail | `drafts.update` | **stays** | the one transport with a draft object of its own, outliving each message it wraps |

  That is the whole reason this is one verb rather than a create and an update: the difference is real, it is not a capability a host could usefully branch on, and the engine absorbing it is the engine doing its job. Graph is the case that proves the contract earns its keep, because the same adapter answers both ways depending on what changed. Gmail's key is a **draft** id and its synced message carries a different one, which is safe only because these two verbs are the sole consumers of what `put_draft` returns.
  - **Rewrite in place wherever the transport allows it.** A saved draft is saved *repeatedly*, so the cost that matters is the cost of the second and hundredth save, not the first. Creating a replacement and removing the original costs two requests, moves the key, and makes the draft jump to the top of the Drafts folder every time; where a transport can rewrite the stored message, that is one request and none of those effects. Graph and Gmail can, and do.

...
- **Storing a draft is two neutral verbs, and the key is the adapter's answer, not the caller's assumption.** `put_draft(account, &Draft, replacing: Option<&ProviderKey>) -> ProviderKey` and `delete_draft(account, &ProviderKey)`, gated by the `mail_drafts` capability. A host hands over the same `Draft` it would submit, plus the draft this one supersedes, and stores **whatever key comes back** — because on three of the four adapters the key *moves* and on one it does not:

  | adapter | a re-save is | key after | why |
  |---|---|---|---|
  | IMAP | `APPEND`, then `UID STORE \Deleted` + `UID EXPUNGE` | **moves** | a stored message is immutable; two commands, no transaction |
  | JMAP | one `Email/set` carrying `create` **and** `destroy` | **moves** | an `Email` is immutable (measured, not assumed), but RFC 8620 §5.3 fixes create-before-destroy, so it is atomic |
  | Graph | `PATCH /me/messages/{id}`, plus the attachment collection | **stays** | a draft's `subject`/`body`/recipients are writable, and an attachment is a resource of its own |
  | Gmail | `drafts.update` | **stays** | the one transport with a draft object of its own, outliving each message it wraps |

  That is the whole reason this is one verb rather than a create and an update: the difference is real, it is not a capability a host could usefully branch on, and the engine absorbing it is the engine doing its job. Gmail's key is a **draft** id and its synced message carries a different one, which is safe only because these two verbs are the sole consumers of what `put_draft` returns.
  - **Rewrite in place wherever the transport allows it, and send only what changed.** A saved draft is saved *repeatedly*, so the cost that matters is the second and hundredth save, not the first. Creating a replacement and removing the original costs two requests, moves the key, and makes the draft jump to the top of the Drafts folder every time. Graph and Gmail can rewrite instead, and Graph goes further: an attachment is a resource of its own there, so a draft that gains or loses one sends just that attachment and keeps its message. Re-uploading every attachment on every save, or replacing the message to change one, is the thing to design out: a draft with a large file attached should not get more expensive each time it is saved. Where a transport *must* replace the message, the attachments can still be carried over: a JMAP replacement references the `blobId`s the stored draft already holds rather than sending the bytes again.
  - **Where it is two calls, the new copy comes first, and a failed removal does not fail the save.** The order decides which way a partial failure falls: append-then-remove leaves the user two drafts, remove-then-append can leave them none, and for a message nobody has sent a duplicate is recoverable while a loss is not. Returning an error after the new copy is stored would be worse still, because the caller retries and appends another.
  - **An absent draft is a successful delete.** The verb sits behind a retryable op, so it *will* be called again on a draft the first call removed; an adapter that reported that as a failure would park the op forever. Getting this right is per-adapter and not guessable: JMAP answers a `notFound` `SetError`, Gmail and Graph answer `404` — **and Graph also answers `403 ErrorCannotDeleteObject`** for a message that existed and was deleted, which is the shape a retry actually meets. Both Graph answers were observed against a real mailbox, in both directions, and are pinned as fixtures; the match is on the error *code*, since a genuine permission failure is also a `403`.
  - **There is no lost-update guard**, and none is claimed. A draft has one author at a time in practice, the transports offer nothing to guard it with, and a `mail_drafts` flag that implied otherwise would be the same mistake `WriteGuard` exists to prevent on the calendar side.
...
- **`concurrent_fetches` is a fact about a *connection*; it is not the account's ceiling.** It
  answers "would overlapping single-object fetches on this connection help or merely
  contend", which is what a caller draining a work list needs. It is deliberately **not** what
  bounds an account: on `provider-imap` it is `1` because one socket is one command at a time,
  and an account holds one socket per folder, so its account capacity is many; on every HTTP
  adapter the server counts against the *account*, so several providers sharing a mailbox
  share one number. Reading the two as the same kind of number serializes IMAP to fix Graph.
  What bounds an account is `engine_http::RequestGate`, one per account, narrowed by the
  adapter as it connects and enforced inside `send_retrying` where every request passes
  (`http-throttling.md`). Both facts come from the adapter, so a host still never switches on
  provider kind.
...
- **A shared mailbox is just another account, so the seam is *discovery* and nothing else.** A credential can often open more than its own mailbox — a Microsoft 365 shared mailbox, a JMAP account shared with the user, an IMAP store another user granted. The engine is already multi-account, so a host onboards such a store under an `AccountId` of its own, binds a provider to it with the same credential, and every sync/store/search path applies unchanged: **no new `SyncScope`, no cursor, no account kind, no SQL migration**. What was missing was the onboarding step — *which stores can I open?* — which is `Provider::list_shared_mailboxes()` and `resolve_shared_mailbox(address)`, returning `SharedMailbox { handle, name, address }` (`engine-provider/src/shared.rs`; the host reaches both through `Engine::list_shared_mailboxes` / `resolve_shared_mailbox`, which checks the typed address before any request). Neither takes an `&AccountId`: like `connection_info()`, what a credential can reach is a fact about the credential, identical whichever store the provider is bound to.
  - **Which verb answers is a capability, because the answer differs in kind.** `Capabilities::shared_mailboxes() -> SharedMailboxes` is `Enumerable` (the server lists them; `resolve` then matches the list — the trait default does that, ASCII-case-insensitively, so JMAP and IMAP implement `list` alone), `ByAddress` (the server answers only for a named address), or `Unsupported`. A host reads it *before* onboarding: `Enumerable` affords a pick-from-a-list flow, `ByAddress` requires the user to type an address. Not a boolean for exactly that reason.
  - **The list never contains the credential's own store, and `resolve` never returns it.** A host renders the list as "add another mailbox" without filtering, and cannot onboard its own mailbox twice by typing its own address.
  - **Binding is per adapter, under one name.** Each adapter takes back the `SharedMailboxId` it handed out: `JmapConfig::with_shared_mailbox`, `ImapConfig::with_shared_mailbox`, `MailboxPrincipal::shared`. The handle is opaque — a JMAP account id, an IMAP namespace path, a Graph address — and a URL or path detail must not leak into a neutral type. An address is not identity: a Microsoft 365 alias resolves to its target mailbox.
  - **A `SharedMailbox` carries no rights and no kind.** Rights, because the account-level signal is unusable — live against Stalwart a JMAP account shared read-only reports `isReadOnly: false` while its only mailbox grants `lr` — so rights live on each folder, `Mailbox::access` (`modeling.md`), once the store is bound and synced. Kind, because Graph's `userPurpose` is the only such vocabulary any adapter publishes and a delegated credential cannot read it for any mailbox but its own (`graph.md`).
  - **The survey, per provider** (every mail adapter, per "Provider-neutral by default"):

    | Adapter | Mechanism | `SharedMailboxes` | Binding |
    |---|---|---|---|
    | JMAP | the session's `accounts` map, `isPersonal: false` (RFC 8620 §1.6.2); on a JMAP Sharing server, the shares the user subscribed to (RFC 9670) | `Enumerable` when the server serves mail and an `accounts` map | every domain the shared account exposes is addressed at it |
    | IMAP | the other-users' and shared namespaces (RFC 2342), one store per owner below each | `Enumerable` when the session has `NAMESPACE` **and** `ACL` — a server without ACLs has no way for one user to share with another | the folder list and every placement are scoped to that store |
    | Graph | none that lists; `/users/{address}` answers or does not | `ByAddress` | `MailboxPrincipal::shared` roots every request at `/users/{address}` |
    | Gmail | delegation is real but needs a service account with domain-wide delegation, never a user token | `Unsupported` | — |
    | CalDAV / CardDAV | not a mail adapter | `Unsupported` | — |

...
- **Tier 3:** full bodies and attachments fetched on demand and cached with quotas. **Implemented** for bodies, split by *text vs bytes* (`store-and-sync.md`): `Provider::fetch_message_source` returns the raw RFC 5322 source, cached lease-free in a content-addressed filesystem blob area (not SQLite); `engine-mime` extracts the displayable text, which is stored in SQLite and **indexed for search** through a lease-free body FTS. Surfaced as `Engine::message_body` (and `search_mail` matches the body). Quota/eviction, per-attachment entities + lazy attachment fetch, and embeddings/RAG over the indexed text are later refinements.
